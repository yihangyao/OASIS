...
class TemporalUnet(nn.Module):
    ...
    def forward(self, x, cond, time, returns=None, use_dropout=True, force_dropout=False, cost_returns=None):
        '''
        ...
        if self.returns_condition:
            ...
            if cost_returns is not None:
                if len(cost_returns.shape) == 1:
                    cost_returns = cost_returns.reshape(-1, 1)

                return_condition = torch.cat([returns, cost_returns], dim=-1)
            else:
                return_condition = returns
            return_condition = return_condition.type(torch.float32)
            returns_embed = self.returns_mlp(return_condition)
            if use_dropout:
...
class MLPnet(nn.Module):
    def __init__(
        ...
        self.returns_condition = returns_condition
        self.cost_return_condition = cost_return_condition
        self.condition_dropout = condition_dropout
        self.calc_energy = calc_energy
        ...
        if self.returns_condition:
            return_condition_dim = 1
            if self.cost_return_condition:
                return_condition_dim = 2
            self.returns_mlp = nn.Sequential(
                        nn.Linear(return_condition_dim, dim),
                        act_fn,
                        nn.Linear(dim, dim * 4),
                        act_fn,
                        nn.Linear(dim * 4, dim),
    ...
    def forward(self, x, cond, time, returns=None, use_dropout=True, force_dropout=False, cost_returns = None):
        ...
        if self.returns_condition:
            assert returns is not None
            if self.cost_return_condition:
                assert return_conditions is not None
                return_conditions = torch.concatenate(
                    (returns, return_conditions), 0
                )
            else:
                return_conditions = returns
            returns_embed = self.returns_mlp(return_conditions)
            if use_dropout:
                mask = self.mask_dist.sample(sample_shape=(returns_embed.size(0), 1)).to(returns_embed.device)
                returns_embed = mask*returns_embed
            if force_dropout:
